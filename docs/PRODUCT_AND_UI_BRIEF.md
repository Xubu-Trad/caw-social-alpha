> Alpha.14 revision: [Independent permission inspection](INDEPENDENT_PERMISSION_READER.md) adds delegated and owner-granted records, exact spending limits and terminal cancellation checks to the separate history engine. It imports no writer permission or accounting helpers. [Current authority](AUTHORITY_AND_FRESHNESS.md) remains a proposed design, not an implemented proof. Inherited history and live permission restoration remain unsupported; source conflicts stay open. Earlier reviews below are historical.
