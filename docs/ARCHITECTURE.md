> Alpha.12 revision: [Signed-action interoperability](INDEPENDENT_ACTION_READER.md) adds a separately written, read-only Node reader and fixed byte fixtures. It imports no application helpers. This checks the bounded synthetic action format, not whole-history replay, wallet authority, newest state or a separate cryptographic provider. Existing UI code and source conflicts are unchanged; earlier reviews below are historical.
