> Alpha.9 revision: [Signed owner cancellation](OWNER_CANCELLATION.md) closes the local grant and is independently checked during record recovery. Cancellation changes no model nonce, fee or balance. Older snapshots cannot prove current permission. Real NFT authority, persistent grant lifecycle and trusted shared freshness remain unimplemented. All source conflicts remain open; earlier reviews below are historical.
