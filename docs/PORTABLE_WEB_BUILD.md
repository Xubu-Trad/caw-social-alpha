...
Run npm run build (or node scripts/build.mjs). dist/ contains fourteen allowlisted public assets and SHA256SUMS.txt. All application asset and configuration links are relative. The local server also exposes an exact /mirror/ prefix for relocation testing; it is the same process, not an independent operator.
