...
The [integration tests](../tests/caw-checked-observer.test.mjs) reuse retained controlled local captures and explicitly synthetic mutations. No new live acquisition, provider independence, public-chain consensus or external audit is claimed. Alpha.37 adds an explicit [provider comparison mode](HEADER_CHECKED_PROVIDERS.md) using this checked observer. The earlier comparison export and Python acquisition coordinator retain their earlier paths; the frontend remains a simulation. [Authentication, body inclusion and live integration](ROADMAP.md) remain open gates.
