"""Bounded local Anvil runner. No wallet, signing or remote transaction transport.
...
FIXTURE_SHA256 = '7e902c32c0bf5027bf84abb738d23808aae3e08f301b0fa26ef7b6bf7d77b38d'
RUNTIME_SHA256 = 'e6e91d3af6e3d170b77f93fcfcd1503526e86dbfcb4be1d12cfcbf0c952eea98'
...
ACTORS = frozenset(('0x0000000000000000000000000000000000000001',
...
class Node:
    ...
    def _params(self, method, params):
        ...
        if method == 'anvil_setBalance' and len(params) == 2 and params[0] in ACTORS and _quantity(params[1], 10 ** 18):
            return
        if method == 'anvil_setCode' and self.mode == 'synthetic' and len(params) == 2 and params[0] == TOKEN:
            build_path = Path(__file__).with_name('fixture-build.json')
            raw = build_path.read_bytes()
            if len(raw) > MAX_RESPONSE or hashlib.sha256(raw).hexdigest() != FIXTURE_SHA256:
                _fail('FIXTURE_BUILD_MISMATCH')
            build = _json(raw)
            code = params[1]
            if (not isinstance(code, str) or not _HEX.fullmatch(code) or len(code) != 3376 * 2 + 2
                    or build.get('runtime') != code or build.get('runtime_sha256') != RUNTIME_SHA256
                    or hashlib.sha256(bytes.fromhex(code[2:])).hexdigest() != RUNTIME_SHA256):
                _fail('FIXTURE_RUNTIME_MISMATCH')
            return
        ...
        if method in ('eth_sendTransaction', 'eth_call'):
            ...
            if (not isinstance(transaction, dict) or set(transaction) - {'from', 'to', 'gas', 'gasPrice', 'data', 'value', 'nonce'}
                    or transaction.get('from') not in ACTORS or not _quantity(transaction.get('gas'), 1000000)
    ...
    def receipt(self):
        return _json(_encoded({'schema': 'caw-local-node/1', 'mode': self.mode,
            ...
            'limits': {'node_process_bytes': MEMORY_LIMIT, 'node_seconds': 600, 'output_bytes': MAX_RESPONSE,
                       'response_bytes': MAX_RESPONSE, 'trace_bytes': MAX_TRACE, 'local_requests': 1800,
